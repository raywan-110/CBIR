...
class RandomProjection(object):
    ...
    def _concat_feat(self, db, feats):
        ...
        for idx in range(len(samples)):
            for feat in feats[1:]:
                ...
                samples[idx]['hist'] = np.append(samples[idx]['hist'],
                                                 feat[key]['hist'])
    ...
    def _get_eps(self, n_samples, n_dims, n_slice=int(1e4)):
        ...
        for i in range(1, n_slice):
            eps = i / n_slice
            jl_dim = johnson_lindenstrauss_min_dim(n_samples=n_samples,
                                                   eps=eps)
            if jl_dim <= new_dim:
                print(
                    "rate %.3f, n_dims %d, new_dim %d, dims error rate: %.4f" %
                    (self.keep_rate, n_dims, jl_dim,
                     ((new_dim - jl_dim) / new_dim)))
                return eps
        return -1
...
if __name__ == "__main__":
    ...
    d_type = 'd1'
    depth = 30
    fusion = RandomProjection(features=['color'],
                              keep_rate=keep_rate,
                              project_type=project_type)
